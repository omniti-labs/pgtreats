...
IF to_char(current_date,'dd') <> '01' THEN
    ...
    select a.table_owner, a.schema_name, a.table_name, a.actual_size, a.actual_size-b.actual_size AS table_growth, 1, a.capture_time
...
IF to_char(current_date,'dd') = '01' THEN
    insert into otools.table_growth (table_owner, schema_name, table_name, actual_size, growth_size, sum_flag, capture_time)
    select a.table_owner, a.schema_name, a.table_name, actual_size, sum(growth_size), 2, (current_date - '1 month'::interval) 
    from otools.table_growth a 
